...
def test_tmdb_details_retains_actor_ids_profiles_director_and_certification() -> None:
    async def run() -> None:
...
def test_actor_id_discovery_uses_tmdb_person_id() -> None:
    async def run() -> None:
        async def handler(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "total_results": 1,
                    "results": [
                        {"id": 123, "title": "Example", "release_date": "2026-08-01"}
                    ],
                },
            )
...
def test_rich_details_ui_has_shared_cast_ratings_and_download_context() -> None:
    ...
    assert "Back to movie details" in html
...
def test_rich_movie_details_keeps_release_lifecycle() -> None:
    ...
    with patch.object(
        rich_details.release_lifecycle,
        "movie_details",
        AsyncMock(return_value=movie.copy()),
    ):
        principal = main.Principal(
            user_id="ha-admin",
            username="stu",
        ...
        result = asyncio.run(
            rich_details.rich_movie_details(123, principal, context="browse")
        )
