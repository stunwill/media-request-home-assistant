...
class TmdbTvClient:
    ...
    async def _get(self, path: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                headers={"User-Agent": "MediaHub/0.10.0"},
            ) as client:
            ...
            message = "TMDb credentials were rejected" if status == 503 else "TMDb TV request failed"
        ...
        *,
        ...
        if query.strip():
            ...
            params = {
                "page": page,
                "include_adult": "false",
                "sort_by": "vote_average.desc" if collection == "top_rated" else "popularity.desc",
            }
        ...
        if query.strip() and filters:
            shows = [
                ...
                if (not genre_id or genre_id in item["genre_ids"])
                and (not year_from or (str(item["year"] or "").isdigit() and int(item["year"]) >= year_from))
                and (not year_to or (str(item["year"] or "").isdigit() and int(item["year"]) <= year_to))
                and (not rating_from or item["rating"] >= rating_from)
                and (not rating_to or item["rating"] <= rating_to)
            ]
        return {
    ...
    async def details(self, tmdb_id: int) -> dict[str, Any]:
        ...
        show.update(
            {
                "last_air_date": str(payload.get("last_air_date") or ""),
                "status": str(payload.get("status") or ""),
                "number_of_seasons": int(payload.get("number_of_seasons") or 0),
                "number_of_episodes": int(payload.get("number_of_episodes") or 0),
                ...
                "external_ids": {
                    ...
                    if key in {"imdb_id", "tvdb_id", "facebook_id", "instagram_id", "twitter_id"} and value
                },
                "cast": [
                    {
                ...
                "seasons": [
                    {
                        ...
                        "name": str(item.get("name") or f"Season {item.get('season_number', '')}"),
...
class SonarrClient:
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                timeout=self.timeout,
                transport=self.transport,
                headers={"X-Api-Key": self.api_key, "User-Agent": "MediaHub/0.10.0"},
            ) as client:
            ...
            status = 503 if error.response.status_code in {401, 403} else 502
            message = "Sonarr credentials were rejected" if status == 503 else "Sonarr request failed"
            raise MediaServiceError(message, status_code=status) from error
        except (httpx.RequestError, ValueError) as error:
    ...
    async def options(self) -> dict[str, Any]:
        ...
        return {
            "root_folders": [item for item in roots or [] if isinstance(item, dict)],
            "quality_profiles": [item for item in profiles or [] if isinstance(item, dict)],
        }
    ...
    async def ensure_series(
        ...
        *,
        ...
        candidate = next(
            (item for item in candidates if tvdb_id and int(item.get("tvdbId") or 0) == tvdb_id),
        ...
        for season in candidate.get("seasons", []) or []:
            ...
            item = dict(season)
            item["monitored"] = number > 0 and (not requested or number in requested)
            seasons.append(item)
        body = {
            **candidate,
        ...
        if seasons:
            for season_number in seasons:
                await self._request(
                    "POST",
                    "/api/v3/command",
                    json={"name": "SeasonSearch", "seriesId": int(series_id), "seasonNumber": season_number},
                )
            return
        await self._request(
            "POST",
