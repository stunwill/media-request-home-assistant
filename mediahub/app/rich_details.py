...
_RICH_DETAILS_UI = r"""
<style>
  .detail-grid{display:grid;grid-template-columns:minmax(0,1fr);gap:18px}.section-title{font-size:.82rem;text-transform:uppercase;letter-spacing:.1em;color:#aeb8c8;font-weight:900;margin:18px 0 10px}.ratings-grid{display:flex;flex-wrap:wrap;gap:10px}.rating-card{min-width:120px;padding:11px 13px;border:1px solid var(--border);border-radius:13px;background:#0c1018;text-decoration:none;color:var(--text)}.rating-card strong{display:block;font-size:.75rem;color:var(--muted);margin-bottom:4px}.cast-strip{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px}.cast-card{border:1px solid var(--border);border-radius:13px;background:#0c1018;color:var(--text);padding:10px;text-align:left;min-height:70px}.cast-card img{width:42px;height:42px;object-fit:cover;border-radius:50%;float:left;margin-right:9px}.cast-name{font-weight:850}.cast-role{font-size:.75rem;color:var(--muted);margin-top:3px}.library-card{padding:14px;border:1px solid var(--border);border-radius:14px;background:#0c1018;display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px}.library-card div span{display:block;color:var(--muted);font-size:.7rem;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px}.download[role=button]:focus,.close:focus,.button:focus,.cast-card:focus,.rating-card:focus{outline:2px solid var(--accent-2);outline-offset:2px}@media(max-width:760px){.cast-strip{grid-template-columns:1fr 1fr}.rating-card{flex:1 1 42%}}
</style>
<script>
  // Keep rich-detail rendering self-contained. The lifecycle helper can be
  // absent when later UI layers replace or recompose the base script.
  function richLifecyclePrimary(movie){
    const lifecycleState=movie?.lifecycle?.state||'released_unknown';
    return ['announced','theatrical_upcoming'].includes(lifecycleState)?'watch':'request';
  }
  ...
  if(baseOpenMovie){openMovie=async function(id){detailTrigger=document.activeElement;return await baseOpenMovie(id);};}
  ...
  renderDetail=function(){const movie=state.movie;const genres=(movie.genres||[]).map(item=>item.name).join(' · ');const backdrop=movie.backdrop_url?`background-image:url('${esc(movie.backdrop_url)}')`:'';const lifecycle=movie.lifecycle||{};const message=movie.lifecycle_message||{};const watch=movie.watch;const primary=richLifecyclePrimary(movie);const downloads=movie.context==='downloads';const statusCard=`<div class="lifecycle-card"><span class="lifecycle-label">${esc(message.label||'RELEASE STATUS')}</span><div class="lifecycle-headline">${esc(message.headline||'Release availability uncertain')}</div><div class="release-date-line">${esc(movie.digital_release_label||'Digital release date not announced')}</div><div class="lifecycle-copy">${esc(message.explanation||'')}</div>${watch&&!downloads?`<div class="release-date-line">Watching for release · next check ${esc(new Date(watch.next_check_at).toLocaleString())}</div>`:''}</div>`;let actionButtons='';if(!downloads){const primaryButton=primary==='watch'?`<button class="button primary" id="watch-release">${watch?'Watching for release':'Watch for release'}</button>`:`<button class="button primary" id="auto-request">Request best release</button>`;const secondary=primary==='watch'?`<button class="button" id="search-anyway">Search anyway</button>`:`<button class="button" id="choose-release">Choose a release</button>`;actionButtons=primaryButton+secondary;}if(movie.trailer_url)actionButtons+=`<a class="button" href="${esc(movie.trailer_url)}" target="_blank" rel="noopener noreferrer">Watch trailer</a>`;document.getElementById('detail').innerHTML=`<div class="detail-hero" style="${backdrop}"><div class="detail-copy"><div class="eyebrow">${esc(genres||'Movie')}</div><h2 id="detail-title">${esc(movie.title)}</h2><div class="muted">${esc(movie.year||'')} ${movie.runtime_minutes?`· ${movie.runtime_minutes} min`:''}${movie.certification?` · ${esc(movie.certification)}`:''}</div>${statusCard}<div class="actions">${actionButtons}</div></div></div><div class="detail-body"><div class="detail-grid"><p class="muted">${esc(movie.overview||'No synopsis is available.')}</p>${movie.director?`<div><span class="muted">Directed by</span> <strong>${esc(movie.director.name||movie.director)}</strong></div>`:''}${richRatings(movie)}${richCast(movie)}${libraryInfo(movie)}<div id="release-area"></div></div></div>`;if(!downloads){const auto=document.getElementById('auto-request');if(auto)auto.addEventListener('click',event=>submitRequest(null,event.currentTarget));const choose=document.getElementById('choose-release');if(choose)choose.addEventListener('click',()=>findReleases(false));const anyway=document.getElementById('search-anyway');if(anyway)anyway.addEventListener('click',()=>findReleases(true));const watchButton=document.getElementById('watch-release');if(watchButton)watchButton.addEventListener('click',()=>watchForRelease(watchButton));}bindRichDetail();};

  const baseCloseModal=closeModal;closeModal=function(){baseCloseModal();if(detailTrigger&&typeof detailTrigger.focus==='function'){detailTrigger.focus();detailTrigger=null;}};document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!document.getElementById('modal').classList.contains('hidden'))closeModal();});
  async function openDownloadDetails(item,card){try{detailTrigger=card;state.movie=await api(`downloads/${item.id}/details`);renderDetail();document.getElementById('modal').classList.remove('hidden');document.getElementById('close-modal').focus();}catch(error){toast(error.message);}}
  function bindDownloadDetailCards(items){document.querySelectorAll('.download').forEach((card,index)=>{const item=items[index];if(!item||card.dataset.richBound)return;card.dataset.richBound='true';card.tabIndex=0;card.setAttribute('role','button');card.setAttribute('aria-label',`View details for ${item.title}`);card.addEventListener('click',()=>openDownloadDetails(item,card));card.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openDownloadDetails(item,card);}});});}
  loadDownloads=async function(silent=false){if(state.downloadLoading)return;state.downloadLoading=true;const container=document.getElementById('downloads');if(!silent)container.innerHTML='<div class="empty">Refreshing Radarr and qBittorrent...</div>';try{const data=await api('downloads');container.innerHTML=data.length?data.map(item=>`<article class="panel download"><div><h3>${esc(item.title)}</h3><div class="muted">Requested by ${esc(item.requested_by_name)} · ${esc(item.status_message||item.status)}</div><div class="progress"><span style="width:${Number(item.progress||0)}%"></span></div></div><span class="status ${esc(item.status)}">${esc(item.status.replaceAll('_',' '))} ${Number(item.progress||0).toFixed(0)}%</span></article>`).join(''):'<div class="empty">No movie requests yet. Browse the catalogue to request one.</div>';bindDownloadDetailCards(data);}catch(error){if(!silent)container.innerHTML=`<div class="empty">${esc(error.message)}</div>`;}finally{state.downloadLoading=false;}};
</script>
"""
